import { SharedAdminNav } from '../SharedAdminNav';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { HeroImageTab } from './tabs/HeroImageTab';
import { GalleryManagementTab } from './tabs/GalleryManagementTab';

export function MediaManagement() {
  return (
    <div className="flex">
      <SharedAdminNav />
      <div className="flex-1">
        <header className="bg-white border-b border-[#e5e7eb] px-8 py-6">
          <div className="flex justify-between items-center">
            <div>
              <h1 className="text-2xl font-bold text-[#00105A] mb-2 m-0">Media Management</h1>
              <p className="text-[#6b7280] m-0">Manage Cloudinary assets and hero images</p>
            </div>
            <div className="text-sm text-[#6b7280]">
              <span className="font-medium">Priority:</span> 📅 Low Frequency Maintenance
            </div>
          </div>
        </header>
        <main className="p-8">
          {/* Tab System */}
          <Tabs defaultValue="hero-images" className="w-full">
            <TabsList className="grid w-full grid-cols-2 bg-[#f9fafb] border border-[#e5e7eb]">
              <TabsTrigger
                value="hero-images"
                className="data-[state=active]:bg-[#C5E7FF] data-[state=active]:text-[#00105A] font-medium"
              >
                📅 Hero Image Manager
              </TabsTrigger>
              <TabsTrigger
                value="gallery-management"
                className="data-[state=active]:bg-[#C5E7FF] data-[state=active]:text-[#00105A] font-medium"
              >
                📅 Gallery Management
              </TabsTrigger>
            </TabsList>
            <TabsContent value="hero-images" className="mt-6">
              <HeroImageTab />
            </TabsContent>
            <TabsContent value="gallery-management" className="mt-6">
              <GalleryManagementTab />
            </TabsContent>
          </Tabs>
        </main>
      </div>
    </div>
  );
}
