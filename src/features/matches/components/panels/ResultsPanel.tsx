...
export function ResultsPanel() {
  ...
  const handleReportClick = async (reportId: string) => {
    ...
    if (reportId.startsWith('http')) {
      ...
      try {
        ...
        if (article) {
          setSelectedArticle({ ...article, id: article._id });
  ...
  useEffect(() => {
    async function fetchResults() {
      setLoading(true);
      try {
        let query = supabase.from('vw_latest_results').select('*');
  ...
  if (Object.keys(groupedResults).length === 0) {
    return (
      <div className="py-8 text-center">
        <p className="text-[rgb(var(--dark-gray))]">No recent results found</p>
    ...
    <>
      ...
      <div className="hidden md:block">
        <div className="space-y-8">
          {Object.entries(groupedResults).map(([monthLabel, monthResults]) => (
            <div key={monthLabel}>
