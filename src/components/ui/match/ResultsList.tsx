...
import { getResults } from '@/lib/fixtures-data';
...
const ResultsList: React.FC<ResultsListProps> = ({
  ...
  const filteredResults = useMemo(() => {
    ...
    return filtered;
  }, [allResults, selectedCompetitions, selectedMonth]);
  
  // Group results by month
  const getResultsByMonth = (matches: Match[]) => {
  ...
  const resultsByMonthData = useMemo(() => {
    const sortedResults = [...filteredResults].sort((a, b) => {
      return new Date(b.date).getTime() - new Date(a.date).getTime(); // Note: Results are sorted in reverse order (newest first)
    });
    
    return getResultsByMonth(sortedResults);
  }, [filteredResults]);
  ...
  const sortedMonths = useMemo(() => {
    return Object.keys(resultsByMonthData).sort((a, b) => {
      ...
      return dateB.getTime() - dateA.getTime(); // Note: Months sorted in reverse order (newest first)
    });
  }, [resultsByMonthData]);
