...
// GET: Dropdown data for match galleries modal
export async function GET() {
  try {
    // Fetch recent completed matches from same view as match reports
    ...
    const dropdownData = {
      recentMatches: recentMatches?.map(match => ({
        value: match.id,
        label: `${match.home_team_name} v ${match.away_team_name} Gallery`,
    ...
    console.error('Match galleries dropdown data fetch error:', error);
    return NextResponse.json({
      success: false,
