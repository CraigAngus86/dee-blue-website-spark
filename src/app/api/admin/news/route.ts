...
export async function POST(request: NextRequest) {
  try {
    ...
    // Title word count validation (8-10 words)
    const titleWordCount = countWords(title);
    if (titleWordCount < 8 || titleWordCount > 10) {
      return NextResponse.json({ error: 'Title should be 8-10 words' }, { status: 400 });
    }
...
// PUT: Update existing article (NEWS ONLY)
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.category === 'matchReport') {
    ...
    // Convert body from plain text to blocks if being updated
    if (updateData.body) {
      updateData.body = textToBlocks(updateData.body);
    }
